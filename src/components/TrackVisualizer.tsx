...
export function TrackVisualizer() {
  ...
  useEffect(() => {
    ...
    // Draw continuous waveform across all tracks
    ctx.clearRect(0, 0, width, height)

    let x = 0
    const segWidth = width / 16

    trackData.forEach((track, i) => {
      if (track.freq === 0) {
        // Divider - draw thin line
        ctx.strokeStyle = 'rgba(255,255,255,0.05)'
        ctx.lineWidth = 1
        ctx.beginPath()
        ctx.moveTo(x + segWidth / 2, 0)
        ctx.lineTo(x + segWidth / 2, height)
        ctx.stroke()
        x += segWidth
        return
      }
    ...
    <section ref={sectionRef} className="py-32 md:py-48 relative overflow-hidden">
      <div className="max-w-6xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-0">
          {trackData.filter(t => t.freq > 0).map((track, i) => (
            <div
              key={track.name}
              className="track-row group flex items-center py-2 border-b border-[var(--border)] hover:bg-surface-hover/20 transition-colors duration-300 cursor-none"
              data-cursor
            >
              {/* Frequency indicator */}
              <div
