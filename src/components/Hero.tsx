...
export function Hero() {
  ...
  useEffect(() => {
    const animate = async () => {
      ...
      // Make content visible immediately so there's no blank hero.
    ...
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-[60] pointer-events-none">
        <div className="max-w-7xl mx-auto px-6 md:px-12 h-7 flex items-center justify-between">
          <span className="hero-hw-label font-mono text-[9px] tracking-[0.25em] uppercase text-light-muted/80" style={{ opacity: 1 }}>
            RSM-001 // COSMIC OBSERVATORY
          </span>
          <span className="hero-hw-label font-mono text-[9px] tracking-[0.15em] uppercase text-light-muted/50 hidden sm:inline" style={{ opacity: 1 }}>
      ...
      <section
        ref={heroRef}
        className="relative min-h-[100dvh] flex items-center justify-center overflow-hidden pt-16 md:pt-0 bg-void"
      >
        <div className="max-w-7xl mx-auto px-6 md:px-10 w-full">
          <div ref={titleRef} className="max-w-3xl">
            <div className="hero-badge section-label mb-6" style={{ opacity: 1 }}>
              <span className="badge badge-signal">Electronic / Synthwave</span>
            </div>

            <h1 className="hero-title text-[clamp(3rem,13vw,8rem)] leading-[0.9] mb-4" style={{ opacity: 1 }}>
              <span className="block hollow-text">Red Shift</span>
              <span className="block text-accent">Mantra</span>
            </h1>

            <p className="hero-tagline font-mono text-[11px] tracking-[0.25em] uppercase text-light-muted mt-8 max-w-md" style={{ opacity: 1 }}>
              The geometry of sound · The transmutation of matter
            </p>

            <div className="hero-tagline hidden md:flex items-center gap-4 mt-10" style={{ opacity: 1 }}>
        ...
        {/* Scroll indicator */}
        <div className="hero-scroll absolute bottom-12 left-6 md:left-12 z-20 flex flex-col items-start gap-2" style={{ opacity: 1 }}>
